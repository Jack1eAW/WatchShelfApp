...
- `lib/features/discover`: media model, repository contract, TMDB/mock repository, search/trending UI
...
3. Data repositories call Dio for live TMDB-style responses or mock fallback data.
4. Watchlist and rating repositories persist local values through `LocalStore`.
5. Controllers update Riverpod state after persistence succeeds.

## State Management

Riverpod owns app state:

- `discoverMediaProvider` loads trending or search results.
